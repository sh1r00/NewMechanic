  ...
  <div class="bg-surface rounded-xl border border-border p-6 shadow-xl w-full max-w-md">
    ...
    <div v-if="isSubmitted" aria-live="polite" aria-atomic="true">
      <div class="text-center py-8" role="status">
        ...
        <h3 class="font-display text-2xl text-text-primary mb-2">{{ $t('contact.form.successHeading') }}</h3>
        <p class="font-body text-text-secondary text-sm mb-6">{{ $t('contact.form.successDescription') }}</p>
        <button
          type="button"
    ...
    <form v-else @submit.prevent="handleSubmit" class="space-y-4">
      <h3 class="font-display text-2xl text-text-primary mb-1">{{ $t('contact.form.heading') }}</h3>
      <p class="font-body text-text-secondary text-sm mb-4">{{ $t('contact.form.description') }}</p>

      <!-- Full Name -->
      <div>
        <label for="fullName" class="block font-body text-sm font-medium text-text-primary mb-1">{{ $t('contact.form.fullName') }} *</label>
        <input
          id="fullName"
      ...
      <!-- Vehicle Make & Model -->
      <div>
        <label for="vehicleMakeModel" class="block font-body text-sm font-medium text-text-primary mb-1">{{ $t('contact.form.vehicleMakeModel') }} *</label>
        <input
          id="vehicleMakeModel"
      ...
      <!-- Email -->
      <div>
        <label for="email" class="block font-body text-sm font-medium text-text-primary mb-1">{{ $t('contact.form.email') }} *</label>
        <input
          id="email"
          v-model="form.email"
          type="email"
      ...
      <!-- Phone -->
      <div>
        <label for="phone" class="block font-body text-sm font-medium text-text-primary mb-1">{{ $t('contact.form.phone') }} *</label>
        <input
          id="phone"
          v-model="form.phone"
          type="tel"
      ...
      <!-- Contact Preference -->
      <div>
        <label for="contactPreference" class="block font-body text-sm font-medium text-text-primary mb-1">{{ $t('contact.form.contactPreference') }}</label>
        <select
      ...
      <!-- Description -->
      <div>
        <label for="description" class="block font-body text-sm font-medium text-text-primary mb-1">{{ $t('contact.form.problemDescription') }} *</label>
        <textarea
          id="description"
